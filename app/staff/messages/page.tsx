...
export default function StaffMessagesPage() {
  ...
  const otherUsers = users.filter((u: any) => u.id !== userId);
  const filteredUsers = otherUsers.filter((u: any) => !search || u.displayName?.toLowerCase().includes(search.toLowerCase()) || u.email?.toLowerCase().includes(search.toLowerCase()));
  ...
  const selectedProfile = otherUsers.find((u: any) => u.id === selectedUser) as Record<string, any> | undefined;

  return (
    <div className="flex h-[calc(100vh-8rem)] gap-0 bg-white rounded-xl border border-border overflow-hidden">
      {/* Sidebar */}
      <div className="w-72 border-r border-border flex flex-col shrink-0">
        <div className="p-3 border-b border-border">
          <div className="relative"><Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="w-full pl-8 pr-3 py-1.5 rounded-lg border border-border text-sm outline-none" /></div>
        </div>
          ...
          {filteredUsers.map((u: any) => (
            <button key={u.id} onClick={() => setSelectedUser(u.id)} className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-muted-light/50 transition-colors ${selectedUser === u.id ? "bg-primary/5 border-r-2 border-primary" : ""}`}>
              ...
              <div className="min-w-0">
                <div className="text-sm font-medium text-foreground truncate">{u.displayName || u.email}</div>
                <div className="text-xs text-muted capitalize">{u.role || "user"}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        {!selectedUser ? (
          ...
          <>
            <div className="px-4 py-3 border-b border-border flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">{getInitials(selectedProfile?.displayName || selectedProfile?.email || "")}</div>
